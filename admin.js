...
async function inicializarAdmin() {
    ...
    document.getElementById("botaoSair").addEventListener("click", async () => { await supabaseClient.auth.signOut(); window.location.href = "login.html"; });
    ...
    document.getElementById("filtroEmergencia").addEventListener("change", aplicarFiltros);
    await carregarAdmin();
}
...
async function carregarAdmin() {
    ...
    const { data: chamados = [] } = await supabaseClient.from("acionamentos").select("*, idosos(nome)").order("criado_em", { ascending: false }).limit(100);
...
function aplicarFiltros() {
    ...
    const emergencia = document.getElementById("filtroEmergencia").checked;
    ...
    const chamados = chamadosAdmin.filter(c => idsEncontrados.has(c.idoso_id) && (status === "todos" || c.status === status) && (!emergencia || /emerg|alerta/i.test(`${c.status} ${c.destinatarios}`)));
...
async function atualizarStatus(id) { const select = document.querySelector(`.statusSelect[data-chamado="${id}"]`); const { error } = await supabaseClient.from("acionamentos").update({ status: select.value }).eq("id", id); if (error) alert(error.message); else { const chamado = chamadosAdmin.find(c => c.id === id); if (chamado) chamado.status = select.value; select.parentElement.nextElementSibling.textContent = "Atualizado"; } }
...
async function editarCliente(id) {
    ...
    const dados = { ...cliente, ...(endereco || {}) }; const campos = { nomeEdicao: dados.nome, dataNascimentoEdicao: dados.data_nascimento, rgEdicao: dados.rg, cpfEdicao: dados.cpf, telefoneEdicao: dados.telefone, cepEdicao: dados.cep, numeroEdicao: dados.numero, logradouroEdicao: dados.logradouro, bairroEdicao: dados.bairro, cidadeEdicao: dados.cidade, estadoEdicao: dados.estado, complementoEdicao: dados.complemento, familiar1NomeEdicao: f1.nome, familiar1ParentescoEdicao: f1.parentesco, familiar1TelefoneEdicao: f1.telefone, familiar2NomeEdicao: f2.nome, familiar2ParentescoEdicao: f2.parentesco, familiar2TelefoneEdicao: f2.telefone }; Object.entries(campos).forEach(([campo, valorAtual]) => { document.getElementById(campo).value = valorAtual || ""; });
    document.getElementById("edicaoCliente").scrollIntoView({ behavior: "smooth" });
}

async function salvarCliente(evento) {
    evento.preventDefault(); const id = document.getElementById("idClienteEdicao").value;
    ...
    for (const contato of [{ prioridade: 1, prefixo: "familiar1" }, { prioridade: 2, prefixo: "familiar2" }]) { const dados = { nome: valor(`${contato.prefixo}NomeEdicao`), parentesco: valor(`${contato.prefixo}ParentescoEdicao`), telefone: valor(`${contato.prefixo}TelefoneEdicao`) }; const { data: familiar } = await supabaseClient.from("familiares").select("id").eq("idoso_id", id).eq("prioridade", contato.prioridade).maybeSingle(); if (familiar) await supabaseClient.from("familiares").update(dados).eq("id", familiar.id); else await supabaseClient.from("familiares").insert({ ...dados, idoso_id: id, prioridade: contato.prioridade }); }
